/* Autogenerated by ../linuxcnc/bin/halcompile on Sun Apr  7 17:02:22 2024 -- do not edit */
#include "rtapi.h"
#ifdef RTAPI
#include "rtapi_app.h"
#endif
...
#ifdef MODULE_INFO
MODULE_INFO(linuxcnc, "component:cia402_homecomp:homing module template");
...
static int export(char *prefix, long extra_arg) {
    ...
    if(r != 0) return r;
    r = hal_pin_bit_newf(HAL_OUT, &(inst->is_module_p), comp_id,
        "%s.is-module", prefix);
    if(r != 0) return r;
    *(inst->is_module_p) = 1;
...
int rtapi_app_main(void) {
    int r = 0;
    comp_id = hal_init("cia402_homecomp");
    if(comp_id < 0) return comp_id;
    r = export("cia402-homecomp", 0);
    if(r) {
        hal_exit(comp_id);
    } else {
        hal_ready(comp_id);
...
#undef is_module
#undef is_module_ptr
#define is_module_ptr (__comp_inst->is_module_p)
#define is_module (*__comp_inst->is_module_p)


#line 40 "cia402_homecomp.comp"
...
#define HOMING_BASE "/home/scott/git/linuxcnc/src/emc/motion/homing.c"
...
static void custom_read_homing_in_pins(int njoints)
{
    ...
    for (jno = 0; jno < njoints; jno++) {
      addr = &(custom_joint_home_data->custom_jhd[jno]);
      customH[jno].request_cia402_homing = *(addr->request_cia402_homing);   // IO
      customH[jno].is_cia402_homing = *(addr->is_cia402_homing);             // IN
  		customH[jno].cia402_homing_finished = *(addr->cia402_homing_finished); // IN
    }
}
...
static void custom_write_homing_out_pins(int njoints)
{
    ...
    for (jno = 0; jno < njoints; jno++) {
        addr = &(custom_joint_home_data->custom_jhd[jno]);
        *(addr->request_cia402_homing) = customH[jno].request_cia402_homing; // IO
    }
}
...
static custom_home_state_t chomestate[EMCMOT_MAX_JOINTS] = {0};
    ...
    switch (chomestate[joint_num]) {
      case HOME_BEGIN:
        ...
        customH[joint_num].request_cia402_homing = 1;
      ...
      case HOME_RUNNING:
        ...
        if(customH[joint_num].cia402_homing_finished)
